...
from lf_toolkit.parse.set import SetParser, Set, Term, Intersection, Union, Difference, SymmetricDifference, Complement, Group
...
@pytest.mark.parametrize("input,expected", [
    ...
    # complex
    ("(A n B) u (C - D)", Union(Group(Intersection(Term("A"), Term("B"))), Group(Difference(Term("C"), Term("D"))))),
    ("(A - B) ^ C'", SymmetricDifference(Group(Difference(Term("A"), Term("B"))), Complement(Term("C")))),
])
...
@pytest.mark.parametrize("input,expected", [
    ...
    ("\\overline{(A \\cap B)}", Complement(Group(Intersection(Term("A"), Term("B"))))),
    ("(A \\cup B)", Group(Union(Term("A"), Term("B")))),
    # complex
    ("(A \\cap B) \\cup (C \\backslash D)", Union(Group(Intersection(Term("A"), Term("B"))), Group(Difference(Term("C"), Term("D"))))),
    ("(A \\backslash B) \\triangle \\bar{C}", SymmetricDifference(Group(Difference(Term("A"), Term("B"))), Complement(Term("C")))),
])
